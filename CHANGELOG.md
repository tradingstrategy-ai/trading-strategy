...
- Set up the Vaults Pro (Creem) licence key the same way as the base API key. `Client.create_jupyter_client(needs_vault_data=True)` resolves the key from `settings.json` if present, otherwise the `VAULT_PRO_API_KEY` environment variable, otherwise an interactive prompt, and persists it back to `settings.json` next to the base key so later runs reuse it without prompting; `create_live_client` reuses the stored key non-interactively. `Configuration` gains a `vault_pro_api_key` field (old settings files still load) and `DefaultClientEnvironment.ensure_vault_pro_api_key()` performs the onboarding. The `VAULT_PRO_API_KEY` env var name moves to the dependency-free `tradingstrategy.vault_pro` module (re-exported from `vault_data_client`), so reading it during client setup no longer imports the optional vault dataset dependencies (`zstandard`) that are absent in minimal Pyodide builds (2026-09-04)

- Add `Client.has_vault_data_access()` (with a `BaseClient` default of `False`), so optional vault features such as trade-executor's repair-time metadata cache refresh can skip cleanly on deployments that do not subscribe to the vault datasets, instead of failing on a licence key they never needed (2026-09-01)

- Vault metadata and vault share price history now download through the new `VaultDataClient` in `tradingstrategy.vault_data_client`, which reads the Creem licence key from `VAULT_PRO_API_KEY`. The datasets moved behind a paid API at `https://tradingstrategy.ai/vaults/datasets/download/`, so `Client.fetch_vault_universe()`, `Client.fetch_vault_price_history()` and their `CachedHTTPTransport` counterparts are removed, along with the `VAULT_JSON_BLOB_URL` and `CLEANED_VAULT_PRICE_PARQUET_URL` constants. `Client` carries the licence key, given to `Client.create_live_client(vault_pro_api_key=...)` or falling back to the environment variable, and hands out a configured `VaultDataClient` through `BaseClient.get_vault_data_client()`, so a strategy module that only receives a client can still reach the datasets. Note the licence key is neither the `secret-token:` oracle API key nor a `creem_` prefixed Creem merchant key, both of which the API answers with `403 Invalid API key`. The new API sends no `ETag` and no `Last-Modified` and marks responses `cache-control: private, no-store`, so there is nothing to revalidate an expired cache against: the cache policy is now a plain 12 hour local time to live, tracked as a TODO until the API exposes a validator. Downloads are written to a temporary file and moved into place, so an interrupted transfer can no longer replace a good cache entry with a truncated one. The local cache file is now named `vault-prices.parquet` rather than `vault-price-history.parquet` - the old file is left behind and can be deleted (2026-09-01)

- Add forward-compatible vault redemption-status metadata without conflating live availability, static adapter support or deposit permission (2026-07-31)
- Add `tradingstrategy.binance.price.fetch_binance_price()`: fetch candles for any Binance spot symbol (default `BTCUSDT`, daily) from the public klines REST API, cached incrementally in a local DuckDB file (`~/.tradingstrategy/binance-price.duckdb`) so repeated calls only fetch candles newer than the cached maximum. Automatic fallback to the public market-data mirror when the main endpoint is geo-blocked; naive UTC timestamps throughout. Intended as a clean centralised-exchange reference price (e.g. for computing beta of vault returns to BTC and the regression residuals) (2026-07-24)
...
- Add: `Client.fetch_vault_universe()` loads vault universe with full metadata from JSON blob with 24h caching (2026-02-08)
