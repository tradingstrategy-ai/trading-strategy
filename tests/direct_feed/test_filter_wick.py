...
def test_fix_dex_price_data():
    """Run fix_dex_price_data for some sample data"""
    ...
    feed = SyntheticTradeFeed(
        ["ETH-USD", "BTC-USD"],
        {"ETH-USD": TrustedStablecoinOracle(), "BTC-USD": TrustedStablecoinOracle()},
        mock_chain,
        timeframe=timeframe,
        start_price_range=1000,
        end_price_range=1000,
    ...
    for ticker in ("ETH-USD", "BTC-USD"):
        candles = candle_feed.get_candles_by_pair(ticker)
        candles = prepare_raw_candle_data(candles)
        candles["pair_id"] = ticker
    ...
    fixed_df = fix_dex_price_data(df, freq=timeframe.freq, forward_fill=True)
    assert len(df) == len(fixed_df)
    assert len(fixed_df.get_group("ETH-USD")) == 201