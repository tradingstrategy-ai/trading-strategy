...
def test_vault_universe_with_metadata(tmp_path: Path) -> None:
    ...
    # 1. Download vault metadata into a pytest-provided temporary location.
    vault_universe = VaultDataClient(download_root=download_root).fetch_vault_universe()
    ...
    # 3. Confirm the downloaded JSON was stored under the requested root.
    assert (download_root / "vault-metadata.json").exists()
